...
namespace Falltergeist::VFS {
    DatArchiveStreamWrapper::DatArchiveStreamWrapper(const std::string& path) {
        ...
        if (!_stream.is_open()) {
            throw std::runtime_error("DatArchiveStreamWrapper - can't open _stream: " + path);
        }
        ...
        if (fileSizeInDatFile != _actualFileSize) {
            throw std::runtime_error("DatArchiveStreamWrapper - wrong file size: " + std::to_string(fileSizeInDatFile) +
